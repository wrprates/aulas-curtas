...
Ajusta o nome das colunas com problema de formação, espaços e caracteres inválidos
...
```{r, echo = TRUE}
library(janitor)
df <- data.frame(caseID = 1, DOB = 2, Other = 3, "Coluna com Espaço" = 4)
clean_names(df)
```

## Janitor - `get_dupes`

Apresenta os dados duplicados de um data frame:
```{r, echo=TRUE}
library(janitor)
...
Compara as colunas de diferentes data frames e seus tipos:
```{r, echo=TRUE}
library(dplyr)
...
Executa a mesma comparação retornando verdadeiro ou falso para a consulta:
```{r, echo=TRUE}
library(dplyr)
df1 <- data.frame(a = 1:2, b = c("pequeno", "grande"))
df2 <- data.frame(a = 10:12, b = c("médio", "pequeno", "grande"), c = 0, stringsAsFactors = TRUE) # no exemplo, a coluna b é um fator
...
Para casos como a limpeza de arquivos do Excel que contêm linhas e colunas vazias após serem lidos no R
